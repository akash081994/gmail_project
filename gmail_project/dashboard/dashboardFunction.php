...
class dashboardFunction
{
    ...
    public function updateProfile($first1, $last1, $user1, $email1, $phone1, $location1, $id1, $imgname1, $tmp1)
    {
        $update = mysqli_query($this->dbcon, "update users set first_name='$first1', last_name='$last1', username='$user1', email = '$email1', phone_no='$phone1', location='$location1' where id=$id1") or die('users data not updated');
        $update_photo = mysqli_query($this->dbcon, "update user_photo set username='$user1', pic='$imgname1' where username='$user1'") or die('users photo not updated');
        if($update_photo)
        {
            mkdir("images/$user1", 0777, true);
            chmod("images/$user1", 0777);
            move_uploaded_file($tmp1,"images/$user1/$imgname1");

        }
        if($update && $update_photo)
        {
            echo "<div class='alert alert-success'>Profile updated successfully</div>";
        }
        else
        {
            echo "<div class='alert alert-success'>Profile not updated </div>";
        }
    }
}
